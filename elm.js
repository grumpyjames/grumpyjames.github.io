...
Elm.Functions.make = function (_elm) {
   ...
   if (_elm.Functions.values)
   return _elm.Functions.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Functions";
   var chain = F3(function (f,
   g,
   a) {
      return A2(g,a,f(a));
   });
   _elm.Functions.values = {_op: _op
                           ,chain: chain};
   return _elm.Functions.values;
};
         ...
         return function () {
            ...
            {case "Zoom":
               return A2($Basics._op["++"],
                 ...
                 ".png"))))));}
            _U.badCase($moduleName,
            "on line 27, column 8 to 144");
         }();
      }();
   });
   var tileSize = 256;
   var locate = $CommonLocator.common(tileSize);
   var centeredOn = F2(function (zoom,
   geopt) {
      return function () {
         var os = A2(locate,
         zoom,
...
Elm.SlippyMap.make = function (_elm) {
   ...
   txt) {
      return function () {
         ...
         upC,
         cn);
         var down = A2($Graphics$Element.color,
         downC,
         cn);
         var hover = A2($Graphics$Element.color,
         hoverC,
         cn);
                              ...
                              return _U.replace([["mouseState"
                                                 ,{ctor: "_Tuple2"
                                                  ...
                                                  ,_1: {ctor: "_Tuple2"
                                                       ...
                                                       ,_1: _v5._1._1}}]],
                                model);
                              case true: switch (_v11._1.ctor)
                                {case "_Tuple2":
                                   return function () {
                                        var newModel = A2(applyDrag,
                                        model,
                                        {ctor: "_Tuple2"
                                        ,_0: _v11._1._0 - _v5._1._0
                                        ,_1: _v5._1._1 - _v11._1._1});
                                        return _U.replace([["mouseState"
                                                           ,{ctor: "_Tuple2"
                                                            ,_0: _v5._0
                                                            ,_1: {ctor: "_Tuple2"
   ...
   var events = function () {
      ...
      M,
      $Movement.mouseState);
      var keys = $Signal.map(K)($Signal.map($Tuple.multiplyT({ctor: "_Tuple2"
                                                             ,_0: 256
...
Elm.Tile.make = function (_elm) {
   ...
   $Tuple = Elm.Tuple.make(_elm),
   $Types = Elm.Types.make(_elm);
   var cartesianProduct = function (_v0) {
      return function () {
         switch (_v0.ctor)
         {case "_Tuple2":
   ...
   var flipY = $Tuple.multiplyT({ctor: "_Tuple2"
                                ,_0: 1
                                ,_1: -1});
   var relativeTilePosition = F3(function (tileSize,
   originTile,
   tile) {
      return function () {
         var relativeTile = A2($Tuple.subtractT,
         tile.coordinate,
         originTile.coordinate);
         var position = flipY($Tuple.mapT(F2(function (x,
         y) {
            return x * y;
         })(tileSize))(relativeTile));
         return $Types.Position(position);
      }();
   });
   ...
   var addP = lift2($Tuple.addT);
   var doMove = F5(function (tileSize,
   originTile,
   globalOffset,
   tile,
   element) {
      return function () {
         var tileSpecificOffset = A3(relativeTilePosition,
         tileSize,
         originTile,
         tile);
         var distance = A2(addP,
         globalOffset,
         tileSpecificOffset);
         return $Graphics$Collage.move(A2($Tuple.mapT,
         $Basics.toFloat,
         distance.pixels))($Graphics$Collage.toForm(element));
      }();
   });
   var globalPixelOffset = F3(function (tileSize,
   tileCounts,
   centrePixel) {
      return function () {
         var originPixelOffsets = A2($Tuple.mapT,
         function (a) {
            return tileSize * (a / -2 | 0);
         },
         tileCounts);
         var pixelOffsets = A2($Tuple.subtractT,
         {ctor: "_Tuple2"
         ,_0: 128
         ,_1: 128},
         centrePixel.pixels);
         return $Types.Position(flipY(A2($Tuple.addT,
         originPixelOffsets,
         pixelOffsets)));
      }();
   });
   var originTile = F2(function (centreTile,
   tileCounts) {
      return $Types.Tile(A2($Tuple.subtractT,
      centreTile.coordinate,
      A2($Tuple.mapT,
      vid(2),
      tileCounts)));
   });
   var tileEl = F4(function (size,
   url,
   zoom,
   tile) {
      return A2($Graphics$Element.image,
      size,
      size)(A2(url,zoom,tile));
   });
   ...
   m) {
      return function () {
         ...
         var tileCounts = A2($Tuple.mapT,
         requiredTiles,
         window);
         var globalOffset = A3(globalPixelOffset,
         m.tileSource.tileSize,
         tileCounts,
         mapCentre.position);
         var origin = A2(originTile,
         mapCentre.tile,
         tileCounts);
         var draw = A2($Functions.chain,
         A3(tileEl,
         m.tileSource.tileSize,
         ...
         origin,
         globalOffset));
         var tiles = cartesianProduct(A3($Tuple.mergeT,
         range,
         origin.coordinate,
         tileCounts));
         return $Graphics$Element.layers(_L.fromArray([A2($Basics.uncurry,
                                                      $Graphics$Collage.collage,
                                                      window)(A2($List.map,
                                                      function ($) {
                                                         return draw($Types.Tile($));
                                                      },
                                                      tiles))
                                                      ,A2($Basics.uncurry,
      ...
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 ...
                 _U.badCase($moduleName,
                 "on line 9, column 32 to 50");
              }();}
         _U.badCase($moduleName,
         "on line 9, column 32 to 50");
      }();
   });
   var addT = mergeT(F2(function (x,
   y) {
      return x + y;
   }));
   var subtractT = mergeT(F2(function (x,
   y) {
      return x - y;
   }));
   var multiplyT = mergeT(F2(function (x,
   y) {
      return x * y;
   }));
   var mapT = F2(function (f,_v8) {
      return function () {
         switch (_v8.ctor)
         {case "_Tuple2":
            return {ctor: "_Tuple2"
                   ...
                   ,_1: f(_v8._1)};}
         _U.badCase($moduleName,
         "on line 6, column 20 to 30");
      }();
   });
   var zeroT = {ctor: "_Tuple2"
               ,_0: 0
               ,_1: 0};
